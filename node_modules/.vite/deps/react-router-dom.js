...
function createRouter(init) {
  ...
  async function handleLoaders(request, location, matches, overrideNavigation, submission, replace, pendingActionData, pendingError) {
    ...
    let [matchesToLoad, revalidatingFetchers] = getMatchesToLoad(state, matches, submission, location, isRevalidationRequired, cancelledDeferredRoutes, cancelledFetcherLoads, pendingActionData, pendingError, fetchLoadMatches);
    ...
    if (redirect3) {
      await startRedirectNavigation(state, redirect3, replace);
      return {
        shortCircuited: true
      };
    }
  ...
  async function startRedirectNavigation(state2, redirect3, replace) {
    if (redirect3.revalidate) {
      isRevalidationRequired = true;
    }
    let redirectLocation = createLocation(state2.location, redirect3.location);
    invariant(redirectLocation, "Expected a location on the redirect navigation");
    if (redirect3.external && typeof window !== "undefined" && typeof window.location !== "undefined") {
      if (replace) {
    ...
    if (redirectPreserveMethodStatusCodes.has(redirect3.status) && formMethod && isSubmissionMethod(formMethod) && formEncType && formData) {
...
function DefaultErrorElement() {
  ...
  return React.createElement(React.Fragment, null, React.createElement("h2", null, "Unhandled Thrown Error!"), React.createElement("h3", {
    ...
    style: preStyles
  }, stack) : null, React.createElement("p", null, "\u{1F4BF} Hey developer \u{1F44B}"), React.createElement("p", null, "You can provide a way better UX than this when your app throws errors by providing your own\xA0", React.createElement("code", {
    style: codeStyles
  }, "errorElement"), " props on\xA0", React.createElement("code", {
    style: codeStyles
  }, "<Route>")));
}
var RenderErrorBoundary = class extends React.Component {
  ...
  render() {
    return this.state.error ? React.createElement(RouteErrorContext.Provider, {
...
function useLoaderData() {
  ...
  !thisRoute.route.id ? true ? invariant(false, 'useLoaderData can only be used on routes that contain a unique "id"') : invariant(false) : void 0;
...
function RouterProvider(_ref) {
  ...
  return React.createElement(DataRouterContext.Provider, {
    value: {
      router,
      navigator,
      static: false,
    ...
    navigator
  }, router.state.initialized ? React.createElement(Routes, null) : fallbackElement)));
}
...
function useSearchParams(defaultInit) {
  ...
  let location = useLocation();
  let searchParams = React2.useMemo(() => getSearchParamsForLocation(location.search, defaultSearchParamsRef.current), [location.search]);
  let navigate = useNavigate();
